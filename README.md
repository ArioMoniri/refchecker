...
- **v0.9.24** — **Honesty & report/UI-parity fixes.** (1) **No more "Unknown mismatch"** — cached-verification warnings now render their real message instead of a blank label. (2) **The downloaded report now matches the app exactly** — cosmetic / style-suppressed issues the UI already hides no longer re-surface only in the exported file. (3) **No invented author names** — the extractor preserves cited initials verbatim (`Morgan KA` stays `Morgan KA`) instead of expanding an initial into a fabricated first name, which had caused false "author mismatch" flags. (4) **Transient checker outages** (e.g. a momentary DBLP/API timeout) are no longer reported as citation problems — a genuine "not found" stays honest; a temporary blip says "please retry".
